...
struct fs::_Dir : _Dir_base {
        ...
        : _Dir_base(fdcwd(), p.c_str(), skip_permission_denied, nofollow, ec)
    {
        ...
        if (filename_only) return; // Do not store path p when we aren't going to use it.
#endif
    ...
    pair<int, const posix::char_type*> dir_and_pathname() const noexcept
    {
        const fs::path& p = entry.path();
#if _GLIBCXX_HAVE_DIRFD
        if (!p.empty()) return {::dirfd(this->dirp), std::prev(p.end())->c_str()};
#endif
    ...
    bool do_unlink(bool, error_code& ec) const noexcept
    {
...
namespace {
    template <typename Bitmask> inline bool is_set(Bitmask obj, Bitmask bits) { return (obj & bits) != Bitmask::none; }
...
const fs::directory_entry& fs::directory_iterator::operator*() const noexcept { return _M_dir->entry; }

fs::directory_iterator& fs::directory_iterator::operator++()
{
    if (!_M_dir)
        _GLIBCXX_THROW_OR_ABORT(filesystem_error("cannot advance non-dereferenceable directory iterator", std::make_error_code(errc::invalid_argument)));
    if (!_M_dir->advance()) _M_dir.reset();
    return *this;
}
...
struct fs::recursive_directory_iterator::_Dir_stack : std::stack<_Dir> {
    ...
    path current_path() const
    {
        ...
        if (top().path.empty()) {
            ...
            p = orig;
            for (auto& d : this->c) p /= d.entry.path();
        } else
            p = top().entry.path();
        return p;
    }
...
fs::recursive_directory_iterator::recursive_directory_iterator(const path& p, directory_options options, error_code* ecptr)
{
    ...
    const bool filename_only = is_set(options, __directory_iterator_filename_only);
    ...
    if (dir.dirp) {
        ...
        if (ecptr ? sp->top().advance(skip_permission_denied, *ecptr) : sp->top().advance(skip_permission_denied)) {
            _M_dirs.swap(sp);
        ...
        *ecptr = ec;
        ...
        _GLIBCXX_THROW_OR_ABORT(fs::filesystem_error("recursive directory iterator cannot open directory", p, ec));
}

fs::recursive_directory_iterator::~recursive_directory_iterator() = default;

fs::directory_options fs::recursive_directory_iterator::options() const noexcept { return _M_dirs->options; }

int fs::recursive_directory_iterator::depth() const noexcept { return int(_M_dirs->size()) - 1; }

bool fs::recursive_directory_iterator::recursion_pending() const noexcept { return _M_dirs->pending; }

const fs::directory_entry& fs::recursive_directory_iterator::operator*() const noexcept { return _M_dirs->top().entry; }
...
fs::recursive_directory_iterator& fs::recursive_directory_iterator::operator++()
{
    error_code ec;
    increment(ec);
    if (ec) _GLIBCXX_THROW_OR_ABORT(filesystem_error("cannot increment recursive directory iterator", ec));
    return *this;
}
...
fs::recursive_directory_iterator& fs::recursive_directory_iterator::increment(error_code& ec)
{
    ...
    const bool follow                 = is_set(_M_dirs->options, directory_options::follow_directory_symlink);
    const bool skip_permission_denied = is_set(_M_dirs->options, directory_options::skip_permission_denied);
...
void fs::recursive_directory_iterator::pop()
{
    ...
    if (ec)
        _GLIBCXX_THROW_OR_ABORT(
            filesystem_error(dereferenceable ? "recursive directory iterator cannot pop" : "non-dereferenceable recursive directory iterator cannot pop", ec));
}

void fs::recursive_directory_iterator::disable_recursion_pending() noexcept { _M_dirs->pending = false; }

#if __GNUC__ >= 12
// Used to implement filesystem::remove_all.
fs::recursive_directory_iterator& fs::recursive_directory_iterator::__erase(error_code* ecptr)
{
    error_code ec;
    if (!_M_dirs) {
        ec = std::make_error_code(errc::invalid_argument);
    ...
    if (!ec) {
        ...
        if (_M_dirs->top().advance(skip_permission_denied, ec))
            ...
            _M_dirs->pop();
            if (_M_dirs->empty()) _M_dirs.reset();
            return *this;
        }
    }
    ...
    if (ecptr)
        *ecptr = ec;
        ...
        _GLIBCXX_THROW_OR_ABORT(fs::filesystem_error("cannot remove all", dirs->orig, dirs->current_path(), ec));
